...
def save(fig, stem):
    fig.tight_layout(); fig.savefig(OUT / f"{stem}.pdf", bbox_inches="tight"); fig.savefig(OUT / f"{stem}.png", dpi=300, bbox_inches="tight"); plt.close(fig)


def comparative_heatmap():
    rows = read_csv("experiments/comparative_grid.csv"); e_levels = sorted({float(r["evidence"]) for r in rows}); counts = sorted({int(r["attack_count"]) for r in rows})
    vals = np.zeros((len(counts), len(e_levels))); n = np.zeros_like(vals)
    for r in rows:
        i, j = counts.index(int(r["attack_count"])), e_levels.index(float(r["evidence"]))
...
def ablation_bar():
    ...
    names = list(grouped); means = [np.mean(grouped[n]) for n in names]; cis = [1.96 * np.std(grouped[n], ddof=1) / np.sqrt(len(grouped[n])) for n in names]
    fig, ax = plt.subplots(figsize=(7.0, 4.5)); x = np.arange(len(names)); ax.bar(x, means, yerr=cis, capsize=4)
    ax.set_xticks(x, [n.replace("_", "\n") for n in names]); ax.set_ylabel("Attack-period finalization rate"); ax.set_ylim(0, 1.05)
    ax.set_title("Component ablation: availability"); save(fig, "component_ablation")


def ablation_safety_bar():
    rows = read_csv("experiments/ablation.csv"); grouped = defaultdict(list)
    for r in rows: grouped[r["variant"]].append(float(r["unsafe_certificate_boundary_fraction"]))
    names = list(grouped); means = [np.mean(grouped[n]) for n in names]; cis = [1.96 * np.std(grouped[n], ddof=1) / np.sqrt(len(grouped[n])) for n in names]
    fig, ax = plt.subplots(figsize=(7.0, 4.5)); x = np.arange(len(names)); ax.bar(x, means, yerr=cis, capsize=4)
    ax.set_xticks(x, [n.replace("_", "\n") for n in names]); ax.set_ylabel("Fraction at or below certificate boundary"); ax.set_ylim(0, 1.05)
    ax.set_title("Component ablation: analytical safety boundary"); save(fig, "component_ablation_safety")


def localization_heatmap():
    rows = read_csv("experiments/localization.csv"); locations = sorted({r["location"] for r in rows}); magnitudes = sorted({float(r["magnitude"]) for r in rows})
...
if __name__ == "__main__":
    comparative_heatmap(); ablation_bar(); ablation_safety_bar(); localization_heatmap(); print(f"wrote publication figures to {OUT}")
