...
def comparative_heatmap():
    ...
    d_levels = sorted({float(r["drift"]) for r in rows})
    # Aggregate over location, drift, and seed: no cherry-picking of attack identity.
    vals = np.zeros((len(e_levels), len(d_levels)))
    counts = np.zeros_like(vals)
    for r in rows:
        i = e_levels.index(float(r["evidence"])); j = d_levels.index(float(r["drift"]))
        vals[i, j] += float(r["attack_finalization_difference"]); counts[i, j] += 1
    vals /= counts
    fig, ax = plt.subplots(figsize=(6.6, 4.8))
    im = ax.imshow(vals, origin="lower", aspect="auto")
    ax.set_xticks(range(len(d_levels)), [f"{x:.2f}" for x in d_levels])
    ax.set_yticks(range(len(e_levels)), [f"{x:.2f}" for x in e_levels])
    ax.set_xlabel("Attack drift")
    ax.set_ylabel("Attack evidence")
    ax.set_title("AEGIS minus fixed-quorum attack finalization rate")
    fig.colorbar(im, ax=ax, label="Rate difference")
    save(fig, "comparative_attack_phase")
...
def ablation_bar():
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Component ablation under the same attack trace")
...
def localization_heatmap():
    ...
    im = ax.imshow(vals, origin="lower", aspect="auto", vmin=0, vmax=1)
