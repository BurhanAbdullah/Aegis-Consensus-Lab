...
def run_trace(attack_set, evidence_level, drift_level, seed, params=None):
    ...
    aegis_attack_final = fixed_attack_final = aegis_all_final = fixed_all_final = 0
    ...
    for r in range(ROUNDS):
        ...
        d = {v: 0.0 for v in VALIDATORS}; attacked = ATTACK_START <= r <= ATTACK_END
        ...
        margins_aegis.append(trace.commit_weight - trace.quorum_weight); margins_fixed.append(trace.commit_weight - fixed_threshold)
        if attacked: aegis_attack_final += int(trace.finalized); fixed_attack_final += int(fixed_finalized)
    return {"attack_set": "+".join(attack_set), "attack_count": len(attack_set), "evidence": evidence_level,
            "drift": drift_level, "seed": seed,
