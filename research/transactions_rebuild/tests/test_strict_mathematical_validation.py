...
def test_closed_form_equilibrium_identity():
    ...
    for _ in range(250):
        p = ReducedParams(
            rho=rng.uniform(0.02, 0.25),
            ell=rng.uniform(0.05, 0.35),
            a=rng.uniform(0.20, 0.90),
            c=rng.uniform(0.02, 0.15),
        )
        e = rng.uniform(0.0, 0.8)
...
def test_risk_containment_boundary_is_strictly_respected():
    ...
    # Equality is contained in the mathematical condition.
    assert risk_containment(0.8, 0.0, p)
    # Any positive excess must be rejected.
    assert not risk_containment(0.8, 0.1, p)
...
def test_model_equivalence_full_equilibrium_residual():
    ...
    nxt = full_interior_step(x, e, d, p)
